...
import { resolveTemplate, resolveTemplateLiteral, resolveTemplateObject } from "./resolve-template";
...
export const ConfigItems = z.pipe(
    z.union([
        z.array(ConfigItem), z.extend(ConfigItem, { name: z.nullable(z.optional(z.string())) })
    ]),
    z.transform(
        (val) => {
...
export async function resolveConfig(options: {
    ...
    for (const { name, destination, destinationFormat, values, context } of items) {
        ...
        for (const value of values) {
            ...
            if (value.value) {
                ...
                resolvedValue = await resolveTemplateLiteral(value.valueFrom, deepmerge(options?.context ?? {}, context ?? {}));
            } else if (value.objectFrom) {
                resolvedValue = await resolveTemplateLiteral(value.objectFrom, deepmerge(options?.context ?? {}, context ?? {}));
                ...
                resolvedValue = await resolveTemplate({
                    ...
                    onlyResolved: value.onlyResolved,
            ...
            if (resolvedValue !== undefined) {
                if (value.name === "@") {
                    ...
                    const segments = value.name.includes("__") ? value.name.split("__") : value.name.split(".");
                    while (segments.length > 1) {
                        ...
                        if (typeof edge[segment] !== "object") {
                            throw new Error(`Cannot create tree path at ${value.name}, ${segment} is not an object ${edge[segment]}`);
                        }
                        edge = edge[segment];
                    }
        ...
        if (destination) {
            ...
            const destinationPath = resolve(options.cwd ?? process.cwd(), destination);
