...
test("template with wrong function variables", async () => {
    await resolveTemplateObject({ a: "${myfunc}-thing" }, {}, 'path-to-object').catch((e)=>{
...
test("template with single property", async () => {
    ...
    assert.deepStrictEqual(await resolveTemplateObject(tree, {}, 'a1.dd'), undefined);
    assert.deepStrictEqual(await resolveTemplateObject(tree, {}, 'g1'), 'r');
    assert.deepStrictEqual(await resolveTemplateObject(tree, {}, 'a1.b2'), tree.a1.b2);
    assert.deepStrictEqual(await resolveTemplateObject(tree, {}, 'a1.b2.c3'), tree.a1.b2.c3);
    assert.deepStrictEqual(await resolveTemplateObject(tree, {}, 'a1.b2.c3.d4'), 'e'); 
    assert.deepStrictEqual(await resolveTemplateObject(tree, {}, 'a1.b2.c3.f4'), ['a', 'b', 'c']);
});

const isSSMRunning = fetch("http://localhost:4566").then(res => res.status === 200).catch(()=>false);
